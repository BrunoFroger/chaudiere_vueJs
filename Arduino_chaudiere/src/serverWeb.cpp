...
void initServerWeb(void){
    ...
    server.on("/favicon.ico", HTTP_GET, [](AsyncWebServerRequest * request){
        ...
        request->send(SDFS, "/favicon.ico", "image/png");
    ...
    server.on("/bundle.js", HTTP_GET, [](AsyncWebServerRequest * request){
        ...
        request->send(LittleFS, "/bundle.js", "text/css");   //Fichier en memoire flash
    });
    ...
    server.on("/switchChauffageMode", HTTP_GET, [](AsyncWebServerRequest *request){ 
        ...
        } else if (strcmp(parametre.c_str(), "AUTO") == 0){
            setChauffageMode(MODE_CHAUFFAGE_AUTO);
        } else {
            Serial.println("mode inconnu : " + parametre);
    ...
    server.on("/loadCalendrier", HTTP_GET, [](AsyncWebServerRequest *request){ 
        Serial.println("requete /loadCalendrier");
        //String calendrier = getCalendrierJson();
        //Serial.printf("taille du calendrier : %d\n", calendrier.length());
        //buffer = chargeCalendrier();
        
        // String calendrier = lireFichier("/calendrier.json");
        // Serial.println("Lecture fichier OK");
        // Serial.println(calendrier.length());
        // //request->send(SDFS, "/calendrier.json", "text/json"); });
        serializeJson(calendar, Serial);
        Serial.println("calendrier : " + buffer);
        Serial.println("calendrier envoye");
    ...
    server.on("/getDatasConfig", HTTP_GET, [](AsyncWebServerRequest * request){
        ...
        reponse += "\"adresseIpTempInt\": \"" + String(getIPCapteurTemperatureInterieure()) + "\",\n";
        reponse += "\"adresseIpTempExt\": \"" + String(getIPCapteurTemperatureExterieure()) + "\",\n";
